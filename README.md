...
### Panel web PoC
- `cd panel && npm install && npm run dev` levanta un formulario minimalista (Next.js 16 + Tailwind) que usa el SDK TypeScript y el endpoint `/system/config`. Solo necesitas ingresar `baseUrl` y `token` para validar conectividad antes de construir vistas m�s complejas.
...
- El endpoint ra�z (`http://127.0.0.1:7443/`) responde 404 por dise�o; usa `/system/info` para validar que el agente est� en pie antes de abrir el panel.
