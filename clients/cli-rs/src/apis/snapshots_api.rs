...
pub async fn containers_container_id_snapshots_get(
    configuration: &configuration::Configuration,
    container_id: &str,
) -> Result<Vec<models::Snapshot>, Error<ContainersContainerIdSnapshotsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_container_id = container_id;

    let uri_str = format!(
        "{}/containers/{containerId}/snapshots",
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ContainersContainerIdSnapshotsGetError> =
...
pub async fn containers_container_id_snapshots_post(
    configuration: &configuration::Configuration,
    container_id: &str,
    containers_container_id_snapshots_post_request: Option<
        models::ContainersContainerIdSnapshotsPostRequest,
    >,
) -> Result<models::Task, Error<ContainersContainerIdSnapshotsPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_container_id = container_id;
    let p_body_containers_container_id_snapshots_post_request =
        containers_container_id_snapshots_post_request;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ContainersContainerIdSnapshotsPostError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
...
pub async fn snapshots_snapshot_id_restore_post(
    configuration: &configuration::Configuration,
    snapshot_id: &str,
) -> Result<models::Task, Error<SnapshotsSnapshotIdRestorePostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    let uri_str = format!(
        ...
        configuration.base_path,
        snapshotId = crate::apis::urlencode(p_path_snapshot_id)
    );
    let mut req_builder = configuration
        .client
        .request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SnapshotsSnapshotIdRestorePostError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
